...
if p.NoOfTransfParam <= 7
    ...
    for i = 1:g.nPC
        ...
        [adj, g.adjIdx.prm_opk{i}] = adj.addPrm('x0'   , zeros(3,1), ...
                                                'const', constRotPrm, ...
        ...
        [adj, g.adjIdx.prm_t{i}] = adj.addPrm('x0'   , zeros(3,1), ...
                                              'const', constTransPrm, ...
        ...
        [adj, g.adjIdx.prm_m{i}] = adj.addPrm('x0'   , 1, ...
                                              'const', constScalePrm, ...
    ...
    for i = 1:size(p.PairList,1)
        ...
        if size(CP{i}.X1,1) >= p.NoOfTransfParam % just add correspondences if no. of corr. >= p.NoOfTransfParam
            ...
            % Add conditions
            adj = adj.addCon('fun', @conSimPoint2PlaneOLS, ...
...
if p.NoOfTransfParam == 12
    ...
    for i = 1:g.nPC
        ...
        [adj, g.adjIdx.prm_a{i}] = adj.addPrm('x0', [1 0 0 0 1 0 0 0 1]', ... % a11, a12, a13, a21, a22, a23, a31, a32, a33
                                              'const', constPrm, ...
    ...
    for i = 1:size(p.PairList,1)
        ...
        if size(CP{i}.X1,1) >= p.NoOfTransfParam % just add correspondences if no. of corr. >= p.NoOfTransfParam
            ...
            % Add conditions
            adj = adj.addCon('fun', @conAffPoint2PlaneSimpleOLS, ...
